...
void binary_sorting(int check[100],int arr[100],int i,int j) {
	...
	// bst 노드 삽입 시, 단순 성적순이 아닌 이진탐색법과 같이 구현.	
	int k = (i + j) / 2;
	if (check[k] == 1) {
		return;
	}
...
int main() {
	...
	// binary_sorting 함수에 이용될 두 개의 배열
	int arr[100];
	int check[100];
	
	// arr엔 오름차순의 성적 값을, check는 0으로 초기화
	for (i = 0; i < 100; i++) {
		arr[i] = i;
		check[i] = 0;
	}
	...
	// 입력 받은 값을 key값으로 탐색 진행
	int input_key;
	printf("찾을 학생의 성적을 입력하세요 : ");
	scanf("%d", &input_key);
